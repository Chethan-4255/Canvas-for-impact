...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function collectFormData() {
    ...
    formConfig.questions.forEach(question => {
      ...
      switch (question.type) {
        case 'checkbox':
          // For agreement checkbox, specifically format it as expected by Google Forms
          if (question.id === 'agreement') {
            const checkedBoxes = document.querySelectorAll(`input[name="${question.id}"]:checked`);
            if (checkedBoxes.length > 0) {
              formData[question.id] = 'I agree'; // Exact text that Google Forms expects
            } else {
  ...
  function mapToGoogleForm(formData) {
    ...
    formConfig.questions.forEach(question => {
      if (question.googleFormField && formData[question.id] !== undefined) {
        // Only add non-empty values to the form
  ...
  async function submitToGoogleForm(formData) {
    return new Promise((resolve) => {
      try {
        ...
        // Create an iframe to receive the form submission
        ...
        // Resolve after a timeout - can't detect actual success due to CORS
        setTimeout(() => {
    ...
    submitBtn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          // Show popup instead of the thank you screen
